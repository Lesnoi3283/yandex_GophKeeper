	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				Storage: func(c *gomock.Controller) requiredInterfaces.Storage {
					...
					st.EXPECT().GetLoginAndPassword(gomock.Any(), 1, "example").Return(entities.LoginAndPassword{
						ID:       100,
						Login:    "example",
		...
		},
		{
			...
			fields: fields{
				Storage: func(c *gomock.Controller) requiredInterfaces.Storage {
					st := mocks.NewMockStorage(c)
					st.EXPECT().GetLoginAndPassword(gomock.Any(), 1, "example").Return(entities.LoginAndPassword{}, fmt.Errorf("some storage error"))
					return st
				},
				KeyKeeper: nil,
			},
